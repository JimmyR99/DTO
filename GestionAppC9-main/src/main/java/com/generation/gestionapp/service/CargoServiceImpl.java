...
*/ return null;
    ...
    @Override
    public CargoEditarDTO editarCargoPorId(CargoEditarDTO cargoParaEditar, Long id) {
        Boolean cargoExist = cargoRepository.existsById(id);

        Cargo cargoSeleccionado = cargoRepository.findById(id).get();
        if (cargoExist && cargoParaEditar != null){
            cargoSeleccionado.setNombreCargo(cargoParaEditar.getNombreCargo());
            cargoSeleccionado.setSueldoCargo(cargoParaEditar.getSueldoCargo());
            cargoRepository.save(cargoSeleccionado);
            return cargoParaEditar;
        }
        else{
            return null;
        }
